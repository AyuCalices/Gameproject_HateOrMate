        ...
        private void RequestGridStep(UnitServiceProvider unitServiceProvider, Vector3Int targetTileGridPosition, int skipLastMovementsCount)
        {
            if (PhotonNetwork.IsMasterClient)
            {
                PerformGridStep(unitServiceProvider, targetTileGridPosition, skipLastMovementsCount, 
                    unitServiceProvider.GetService<UnitStatsBehaviour>().GetFinalStat(StatType.MovementSpeed));
            }
            else
            {
            ...
            switch (photonEvent.Code)
            {
                case (int)RaiseEventCode.OnPerformGridStep:
                {
                    ...
                    if (battleData.UnitsServiceProviderRuntimeSet.GetUnitByViewID(viewID, out UnitServiceProvider unitServiceProvider))
                    {
                        ...
                        {
                            UnitBattleBehaviour unitBattleBehaviour = unitServiceProvider.GetService<UnitBattleBehaviour>();
