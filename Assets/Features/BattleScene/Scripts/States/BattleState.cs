...
namespace Features.BattleScene.Scripts.States
{
    ...
    public class BattleState : BaseCoroutineState
    {
        ...
        {
            return battleData.UnitsServiceProviderRuntimeSet.GetUnitsByTag(tagTypes)
                .Any(e =>
                {
        ...
        public override void OnEvent(EventData photonEvent)
        {
            ...
            switch (photonEvent.Code)
            {
                case (int)RaiseEventCode.OnAttack:
                {
                    ...
                    if (battleData.UnitsServiceProviderRuntimeSet.GetUnitByViewID((int) data[0], out UnitServiceProvider unitServiceProvider))
                    {
                        unitServiceProvider.GetService<UnitBattleBehaviour>().BattleClass.AttackCallback((float) data[1], (float) data[2], (UnitClassData_SO) data[3]);
                    }
